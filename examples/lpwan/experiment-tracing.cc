...
public:
  ...
  void CaseRun (uint32_t nLoraEndDevices,
               uint32_t nLoraGateways,
               uint32_t nNbEndDevices,
...
int main (int argc, char *argv[])
{
  ...
  uint32_t nLoraEndDevices = 1;
  uint32_t nLoraGateways = 2;
  uint32_t nNbEndDevices = 100;
  uint32_t nNbGateways = 2;
  double discRadius = 500.0;
  ...
  cmd.AddValue ("nLoraEndDevices", "Number of LoRaWAN class A end devices nodes[Default:30]", nLoraEndDevices);
  cmd.AddValue ("nLoraGateways", "Number of LoRaWAN gateways [Default:1]", nLoraGateways);
  //
  cmd.AddValue ("nNbEndDevices", "Number of LoRaWAN class A end devices nodes[Default:30]", nNbEndDevices);
  cmd.AddValue ("nNbGateways", "Number of LoRaWAN gateways [Default:1]", nNbGateways); 
  //
  ...
  for (uint32_t i = 0; i < nRuns; i++) {
    ...
    simSettings << "\tnLoraEndDevices = " << nLoraEndDevices << std::endl;
    simSettings << "\tnLoraGateways = " << nLoraGateways << std::endl;
    simSettings << "\tnNbEndDevices = " << nNbEndDevices << std::endl;
    ...
    example.CaseRun (nLoraEndDevices, nLoraGateways, nNbEndDevices, nNbGateways, 
        discRadius, totalTime, usPacketSize, usMaxBytes, 
        usDataPeriod, usUnconfirmedDataNbRep, usConfirmedData, dsPacketSize, 
        dsDataGenerate, dsDataExpMean, dsConfirmedData, verbose, 
        stdcout, tracePhyTransmissions, tracePhyStates, traceMacPackets, 
        traceMacStates, traceEdMsgs, traceNsDsMsgs, traceMisc,
        phyTransmissionTraceCSVFileName.str (), phyStateTraceCSVFileName.str (), 
        macPacketTraceCSVFileName.str (), macStateTraceCSVFileName.str (), 
        edMsgTraceCSVFileName.str(), nsDSMsgTraceCSVFileName.str(),  
        miscTraceCSVFileName.str(), nodesCSVFileName.str());
  }
...
void
LoRaWANExampleTracing::CaseRun (uint32_t nLoraEndDevices, uint32_t nLoraGateways, 
    uint32_t nNbEndDevices, uint32_t nNbGateways, double discRadius, 
    double totalTime, uint32_t usPacketSize, uint32_t usMaxBytes, 
    double usDataPeriod, uint32_t usUnconfirmedDataNbRep, bool usConfirmedData,  
    uint32_t dsPacketSize, bool dsDataGenerate, double dsDataExpMean, 
    bool dsConfirmedData, bool verbose, bool stdcout, bool tracePhyTransmissions, 
    bool tracePhyStates, bool traceMacPackets, bool traceMacStates, bool traceEdMsgs, 
    bool traceNsDsMsgs, bool traceMisc,  std::string phyTransmissionTraceCSVFileName, 
    std::string phyStateTraceCSVFileName, std::string macPacketTraceCSVFileName, 
    std::string macStateTraceCSVFileName, std::string edMsgTraceCSVFileName, 
    std::string nsDSMsgTraceCSVFileName, std::string miscTraceCSVFileName, 
    std::string nodesCSVFileName)
{
  m_nLoraEndDevices = nLoraEndDevices;
  m_nLoraGateways = nLoraGateways;
  m_nNbEndDevices = nNbEndDevices;
  ...
  std::cout << "Creating " << (unsigned) m_nNbEndDevices << " NB-IoT devices.\n";
  std::cout << "Creating " << (unsigned) m_nNbGateways << " NB-IoT Gateways.\n";
  m_nbEDNodeContainer.Create(m_nNbEndDevices);
  m_nbGWNodeContainer.Create(m_nNbGateways);
  /* End NB-IoT */
  ...
  loraMobility.Install (m_loraEDNodeContainer);
  ...
  nbMobility.Install (m_nbEDNodeContainer);
  ...
  loraGatewayMobility.Install (m_loraGWNodeContainer);

  Ptr<ListPositionAllocator> nbPositionList = CreateObject<ListPositionAllocator> ();
  for (uint32_t i = 0; i < (m_nNbGateways); i++) {
    Vector v = Vector (-discRadius/2 + (i/2)*discRadius, -discRadius/2 + (i % 2)*discRadius, 0.0);
  ...
  nbGatewayMobility.Install (m_nbGWNodeContainer);

  CreateDevices ();
  m_nbGWDevices = lteHelper->InstallEnbDevice (m_nbGWNodeContainer);
  m_nbEDDevices = lteHelper->InstallUeDevice (m_nbEDNodeContainer);
  //lteHelper->EnableTraces ();

  InstallApplications ();

  // Install the IP stack on the UEs
  internet.Install (m_nbEDNodeContainer);
  Ipv4InterfaceContainer ueIpIface;
  ueIpIface = epcHelper->AssignUeIpv4Address (NetDeviceContainer (m_nbEDDevices));
  // Assign IP address to UEs, and install applications
  for (uint32_t u = 0; u < m_nbEDNodeContainer.GetN (); ++u) {
    Ptr<Node> ueNode = m_nbEDNodeContainer.Get (u);
  ...
  uint32_t half = m_nNbEndDevices / 2;
  for (uint32_t i = 0; i < m_nNbEndDevices; i++) {
    if (half < i)
      lteHelper->Attach (m_nbEDDevices.Get(i), m_nbGWDevices.Get(0));
    else
      lteHelper->Attach (m_nbEDDevices.Get(i), m_nbGWDevices.Get(1));
  ...
  for (uint32_t u = 0; u < m_nbEDNodeContainer.GetN (); ++u) {
    ...
    serverApps.Add (dlPacketSinkHelper.Install (m_nbEDNodeContainer.Get(u)));
    serverApps.Add (ulPacketSinkHelper.Install (remoteHost));
    serverApps.Add (packetSinkHelper.Install (m_nbEDNodeContainer.Get(u)));
    ...
    clientApps.Add (ulClient.Install (m_nbEDNodeContainer.Get(u)));
    if (u+1 < m_nbEDNodeContainer.GetN ()) {
      clientApps.Add (client.Install (m_nbEDNodeContainer.Get(u+1)));
    } else {
      clientApps.Add (client.Install (m_nbEDNodeContainer.Get(0)));
    }
  }
...
{
  std::cout << "Creating " << (unsigned) m_nLoraGateways << " LoRaWAN gateway(s).\n";
  m_loraGWNodeContainer.Create (m_nLoraGateways);

  std::cout << "Creating " << (unsigned) m_nLoraEndDevices << " LoRaWAN class A end devices.\n";
  m_loraEDNodeContainer.Create (m_nLoraEndDevices);

  //m_allNodes.Add (m_loraGWNodeContainer);
  //m_allNodes.Add (m_loraEDNodeContainer);
}
...
LoRaWANExampleTracing::SetupMobility ()
{
  ...
  edMobility.Install (m_loraEDNodeContainer);
  ...
  for (uint32_t i = 0; i < m_nLoraGateways; i++) {
    ...
    //   }
      ...
      NS_FATAL_ERROR (this << " " << m_nLoraGateways << " number of gateways is not supported");
      exit (-1);
    }
  ...
  gwMobility.Install (m_loraGWNodeContainer);
}
...
LoRaWANExampleTracing::CreateDevices ()
{
  ...
  m_loraEDDevices = lorawanHelper.Install (m_loraEDNodeContainer);

  lorawanHelper.SetDeviceType (LORAWAN_DT_GATEWAY);
  m_loraGWDevices = lorawanHelper.Install (m_loraGWNodeContainer);
}
...
LoRaWANExampleTracing::InstallApplications ()
{
  ...
  packetSocket.Install (m_loraEDNodeContainer); // TODO: replace two lines with m_allNodes?
  packetSocket.Install (m_loraGWNodeContainer);
  ...
  for (NodeContainer::Iterator i = m_loraGWNodeContainer.Begin (); i != m_loraGWNodeContainer.End (); ++i)
    {
      Ptr<Node> node = *i;
  ...
  for (NodeContainer::Iterator i = m_loraEDNodeContainer.Begin (); i != m_loraEDNodeContainer.End (); ++i)
  {
      Ptr<Node> node = *i;
...
LoRaWANExampleTracing::OutputNodesToFile ()
{
  ...
  for (NodeContainer::Iterator i = m_loraEDNodeContainer.Begin (); i != m_loraEDNodeContainer.End (); ++i)
  {
    ...
    {
    for (NodeContainer::Iterator it_gw = m_loraGWNodeContainer.Begin (); it_gw != m_loraGWNodeContainer.End (); ++it_gw)
    {
...
LoRaWANExampleTracing::CalculateDataRateIndexPER (Ptr<Application> endDeviceApp)
{
  ...
  Vector posGw;
  for (NodeContainer::Iterator it = m_loraGWNodeContainer.Begin (); it != m_loraGWNodeContainer.End (); ++it)
  {
